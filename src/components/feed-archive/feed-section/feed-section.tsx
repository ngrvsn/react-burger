...
import styles from "./feed-section.module.scss";
...
const FeedSection: FC = () => {
  ...
  const allOrders = useSelector(
    (state: RootState) => state.orderTracking.total
  );
  const totalToday = useSelector(
    (state: RootState) => state.orderTracking.totalToday
  );
  ...
  const inProgressOrders = last10Orders.filter(
  ...
  const inProgressOrdersNumbers = inProgressOrders
