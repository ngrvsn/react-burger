...
const FeedInfo: FC = () => {
  ...
  const updatedOrder = useSelector(
    (state: RootState) => state.order.updatedOrder
  );
  const ingredientsList = useSelector(
    (state: { [prop in string]: TIngredientsState }) =>
      state.ingredients.ingredientList
  );
  const ordersPublic = useSelector(
    (state: RootState) => state.orderTracking.orders
  );
  const ordersUser = useSelector(
    (state: RootState) => state.orderTrackingUser.orders
  );
  ...
  const ordersSplit = (): void => {
    const path = location.pathname.includes("/profile/")
      ? location.pathname.split("/profile/orders/")[1]
      : location.pathname.split("/feed/")[1];

    const selectedOrder = (
      location.pathname.includes("/profile/") ? ordersUser : ordersPublic
    ).find((item: TOrdersSectionProps) => item._id === path);
  ...
  const itemsNumbers = (item: TIngredientProps): number | undefined => {
    if (updatedOrder) {
      const count = updatedOrder.ingredients.filter(
        (e) => e === item._id
      ).length;
      return count;
    }
  ...
  return (
    <main className={styles.main}>
      {updatedOrder && (
        <>
          <h2 className={styles.header}>#{updatedOrder.number}</h2>

          <h2 className={styles.text}>{updatedOrder.name}</h2>
