...
const Feed: FC = () => {
  ...
  const publicOrders = useSelector(
  ...
  const ingredientsInfo = (id: string[]): TIngredientProps[] => {
    const selectedIngredients = ingredientsList.filter((item) =>
      ...
      <section key={item._id} className={styles.section}>
        ...
        >
          ...
          <div className={`${styles.wrapper}`}>
            {!match && (
              <h1 className={styles.state}>{statusText(item.status)}</h1>
            )}
            <div className={`${styles.fragment}`}>
              {displayedIngredients.map((ingredient, key) => (
                <div key={key} className={styles.images}>
  ...
  return (
    <section className={styles.sectionText}>
      {match && <h1 className={styles.headerText}>Лента заказов</h1>}
